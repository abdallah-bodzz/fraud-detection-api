...
def _synthetic_threshold_sweep(
    chosen_threshold: float, chosen_precision: float, chosen_recall: float
):
    """
    ...
    recall = (
        chosen_recall
        + (1 - chosen_recall) * np.exp(-6 * t)
        - (1 - chosen_recall) * np.exp(-6 * chosen_threshold)
    )
...
def build_hero(
    eval_summary: dict, training_summary: dict, generated_from_live_data: bool
) -> plt.Figure:
    ...
    sidebar_bg_ax.add_patch(
        mpatches.Rectangle(
            (0, 0),
            1,
            1,
            transform=sidebar_bg_ax.transAxes,
    ...
    spec_lines = [
        ...
        if brier
        else "calibration     see 03_evaluation.ipynb",
    ]
    ...
    credit_ax.text(
        0,
        0.15,
        "github.com/abdallah-bodzz/fraud-detection-api",
        fontsize=8.5,
        color="#6F8880",
        va="top",
        ha="left",
    ...
    feat_ax.text(
        0,
        1.14,
        "top features by importance (rank order)",
        fontsize=8.0,
        color=LABEL_GRAY,
        va="bottom",
        ha="left",
    ...
    footer_ax.text(
        0,
        0.0,
        "XGBOOST   SCIKIT-LEARN   FASTAPI   DOCKER   PANDAS",
        fontsize=8.5,
        color=LABEL_GRAY,
        va="bottom",
        ha="left",
    ...
    generated_tag = datetime.now(UTC).strftime("%Y-%m-%d")
    source_tag = (
        "live evaluation data"
        if generated_from_live_data
        else "fallback data — run notebooks first"
    )
    footer_ax.text(
        1.0,
        0.0,
        f"v1.0 · GENERATED {generated_tag} · {source_tag.upper()}",
        fontsize=7.5,
        color=LABEL_GRAY,
        va="bottom",
        ha="right",
